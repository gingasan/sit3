...
class Encoder(nn.Module):
    ...
    def forward(self,
                ...
                input_len,
                adjacency):
        layer_outputs, _ = self.transformer(input, input_len, adjacency)  # B x seq_len x h
        if self.use_all_enc_layers:
...
class Transformer(nn.Module):
    ...
    def decode(self,
        ...
        word_rep = self.embedder(code_word_rep,
                                 ...
                                 mode='encoder')
        code_struc_rep = kwargs['code_struc_rep']
        memory_bank, layer_wise_outputs = self.encoder(word_rep, code_len, code_struc_rep)  # B x seq_len x h
