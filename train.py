...
def add_train_args(parser):
    ...
    files.add_argument('--model_dir', type=str, default='../sit_python/',
    ...
    files.add_argument('--train_src', type=str, default='train.token.code',
    ...
    files.add_argument('--train_tgt', type=str, default='train.token.nl',
                       help='Preprocessed train target file')
    files.add_argument('--dev_src', type=str, default='valid.token.code',
                       help='Preprocessed dev source file')
    files.add_argument('--dev_src_tag', nargs='+', type=str,
                       help='Preprocessed dev source tag file')
    files.add_argument('--dev_tgt', type=str, default='valid.token.nl',
                       help='Preprocessed dev target file')
    files.add_argument('--train_guid', type=str, default='train.token.guid',
                       help='Preprocessed train guid file')
    files.add_argument('--dev_guid', type=str, default='valid.token.guid',
                       help='Preprocessed dev guid file')
    files.add_argument('--use_ast', type='bool', default=True,
                       help='Whether to use AST information.')
...
def set_defaults(args):
    ...
    if not args.only_test:
        ...
        if args.use_ast:
            train_guid = os.path.join(data_dir, args.train_guid)
        else:
            train_guid = None
        ...
        if args.use_code_type:
            train_src_tag = os.path.join(args.data_dir, args.train_src_tag)
    ...
    if args.use_ast:
        dev_guid = os.path.join(data_dir, args.dev_guid)
    else:
        dev_guid = None
    ...
    if args.use_code_type:
        dev_src_tag = os.path.join(args.data_dir, args.dev_src_tag)
...
def main(args):
    ...
    if not args.only_test:
        ...
        for train_src, train_src_tag, train_tgt, dataset_name, train_guid in \
    ...
    dev_exs = []
    for dev_src, dev_src_tag, dev_tgt, dataset_name, dev_guid in \
